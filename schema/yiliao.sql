...
CREATE TABLE `jybgb`  (
  ...
  CONSTRAINT `jybgb_ibfk_1` FOREIGN KEY (`YLJGDM`) REFERENCES `mzjzjlb` (`YLJGDM`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `jybgb_ibfk_2` FOREIGN KEY (`YLJGDM`) REFERENCES `zyjzjlb` (`YLJGDM`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `jybgb_ibfk_3` FOREIGN KEY (`JZLSH`) REFERENCES `mzjzjlb` (`JZLSH`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `jybgb_ibfk_4` FOREIGN KEY (`JZLSH`) REFERENCES `zyjzjlb` (`JZLSH`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;
