...
with open('data/mdsql/tables.json', 'r', encoding='utf-8') as file:
    tables = json.load(file)
if len(tables) == 2:
    with open('data/mdsql/tables_gen.json', 'r', encoding='utf-8') as file:
...
with open('data/mdsql/db_content.json', 'r', encoding='utf-8') as file:
...
with open('data/mdsql/all.json', 'r', encoding='utf-8') as file:
    data = json.load(file)
if len(data) == 4340:
    with open('data/mdsql/all_gen.json', 'r', encoding='utf-8') as file:
        data_gen = json.load(file)
    data += data_gen
    for i in range(len(data)):
        data[i]['question_id'] = f'qid{str(i + 1).zfill(5)}'
    with open('data/mdsql/all.json', 'w', encoding='utf-8') as file:
